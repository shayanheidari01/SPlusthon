...
<splusthon.tl.custom.message.Message>`, which means you
...
<splusthon.tl.custom.message.Message.sender>`) don't need an ``await``, but
methods (`message.get_sender
<splusthon.tl.custom.message.Message.get_sender>`) **do** need an ``await``,
...
<splusthon.events.register>` them:
...
You can use `splusthon.events.is_handler` to check if any method is a handler.
...
<splusthon.client.updates.UpdateMethods.add_event_handler>` to the
...
If for any reason you don't want to use `splusthon.events.register`,
...
`events.Raw <splusthon.events.raw.Raw>` for adding, and `None` when
...
.. note::
    ...
    <splusthon.client.updates.UpdateMethods.add_event_handler>`
    if you have used `splusthon.events.register` on the ``callback``
...
it is possible to raise a `splusthon.events.StopPropagation` exception which
...
Remember to check :ref:`splusthon-events` if you're looking for
...
<splusthon.client.telegrambaseclient.TelegramBaseClient.disconnected>` property,
...
<splusthon.client.telegrambaseclient.TelegramBaseClient.disconnected>`
until it completed.
...
<splusthon.client.telegrambaseclient.TelegramBaseClient.disconnected>`,
`client.run_until_disconnected
<splusthon.client.updates.UpdateMethods.run_until_disconnected>` will
...
.. code-block:: python
    ...
    with SoroushClient(..., sequential_updates=True) as client:
        ...
